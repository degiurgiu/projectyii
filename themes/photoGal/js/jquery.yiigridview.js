...
;(function($) {
	...
	$.fn.yiiGridView = function(options) {
		return this.each(function(){
			...
			//*/
			/*######################################################*/
			//fix for yii upgrade to 1.1.14
			var settings = $.extend({}, $.fn.yiiGridView.defaults, options || {}),
			$this = $(this),
			id = $this.attr('id'),
			pagerSelector = '#'+id+' .'+settings.pagerClass.replace(/\s+/g,'.')+' a, #'+id+' .'+settings.tableClass+' thead th a',
    ...
    $.fn.yiiGridView.update = function(id, options, autoselectRow) {
        var settings = $.fn.yiiGridView.settings[id];
        ...
        if (options && options.auto_select !== undefined) {
            auto_select = options.auto_select;
            delete options.auto_select;
        }

        var updateUrl = $.fn.yiiGridView.getUrl(id) + '&' + $.fn.yiiGridView.getFilterData(id, true);
        ...
        options = $.extend({
            ...
            success: function(data,status) {
                ...
                // auto select by id
                if (auto_select && auto_select > 0)
                    $.fn.yiiGridView.autoSelectbyId(id, auto_select);

                // filters & search
                //$.fn.yiiGridView.showFilters(id,data);
                $('#'+id).removeClass(settings.loadingClass);
                $.fn.yiiGridView.selectCheckedRows(id);
                $('#'+id + ' .active-filter-search').empty();
                $('#'+id).closest('div.portlet').find('.portlet-grid-info').remove();
                $.fn.yiiGridView.checkShowActiveFilter(id);
                $.fn.yiiGridView.checkShowActiveSearch(id);

                $("#"+id).initStaticTableHead();
            },
        ...
        if(!$.fn.yiiGridView.settings[id].tagFilter){
        ...
        if(settings.ajaxUpdate!==false) {
            ...
            if(settings.beforeAjaxUpdate !== undefined) {
                options = settings.beforeAjaxUpdate(id, options);
            }
            $.fn.yiiGridView.block(id);

            // adding tags to the data query
            if($.fn.yiiGridView.settings[id].tagFilter)
                options.data = options.data + '&JSearch[tags]=' + $.fn.yiiGridView.settings[id].tagFilter;

            if($.fn.yiiGridView.settings[id].tagOperator)
                options.data = options.data + '&JSearch[tagOperator]=' + $.fn.yiiGridView.settings[id].tagOperator;
            ...
            if(options.type=='GET') {
                ...
                // adding tags to the data query
                if($.fn.yiiGridView.settings[id].tagFilter)
                    options.data.tags = $.fn.yiiGridView.settings[id].tagFilter;

                if($.fn.yiiGridView.settings[id].tagOperator)
                    options.data.tagOperator = $.fn.yiiGridView.settings[id].tagOperator;
    ...
    $.fn.yiiGridView.addTagFilter = function(gridId, tagId, updateOptions) {
        if(!$.fn.yiiGridView.settings[gridId].tagFilter){
            $.fn.yiiGridView.settings[gridId].tagFilter = tagId;
        }
        else{
            $.fn.yiiGridView.settings[gridId].tagFilter = $.fn.yiiGridView.settings[gridId].tagFilter + ',' + tagId;
        }
        console.log(updateOptions);
        $.fn.yiiGridView.update(gridId, updateOptions, $.fn.yiiGridView.settings[gridId].tagFilter);
    };

    $.fn.yiiGridView.removeTagFilter = function(gridId, tagId, updateOptions) {
        if(!$.fn.yiiGridView.settings[gridId].tagFilter){
            return;
        }
        else{
            var tagString = $.fn.yiiGridView.settings[gridId].tagFilter;
            var tagIds = tagString.split(',');
            var tagFilterArray = [];
            $.each( tagIds, function( key, value ){
                if(value != tagId)
                    tagFilterArray.push(value);
            });

            if(!updateOptions){
                updateOptions = {auto_select: $.trim( $('#'+gridId+' .items tr.selected td.col-id').html() ) };
            }

            $.fn.yiiGridView.settings[gridId].tagFilter = tagFilterArray.join(",");
            $.fn.yiiGridView.update(gridId, updateOptions, $.fn.yiiGridView.settings[gridId].tagFilter);
        }
    };

    $.fn.yiiGridView.addTagFiltersArray = function(gridId, tagFilterArray, tagOperator) {
        $.fn.yiiGridView.settings[gridId].tagFilter = tagFilterArray.join(",");
        $.fn.yiiGridView.settings[gridId].tagOperator = tagOperator;
        $.fn.yiiGridView.update(gridId, null);
    };

	/**
	 * Block grid UI and show loading overlay.
	 * The "if" is there because we need it to check if the grid is contained by a portlet, otherwise use a different block method.
	 */
	$.fn.yiiGridView.block = function(id){
		var portletId = $('#'+id).closest('.portlet').attr('id');
		if(portletId){
			$('#'+portletId).blockPortlet();
		}
		else{
			$('#'+id).closest('.portlet-content-footer-container').block({ message: '<div class="grid-loader"></div>',
				css: {border: '0px', backgroundColor: 'transparent'}, 'overlayCSS': {opacity: 0.6}});
		}
	}

	/**
	 * Unblock grid UI and hide loading overlay.
	 * The "if" is there because we need it to check if the grid is contained by a portlet, otherwise use a different block method.
	 */
	$.fn.yiiGridView.unblock = function(id){
		var portletId = $('#'+id).closest('.portlet').attr('id');
		if(portletId){
			$('#'+portletId).unblockPortlet();
		}
		else{
			$('#'+id).closest('.portlet-content-footer-container').unblock();
		}
	}
