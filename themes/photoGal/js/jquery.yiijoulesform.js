...
;(function($) {
		...
		return this.each(function() {
			...
			$.each(settings.attributes, function(i,attribute){
				settings.attributes[i] = $.extend({
					...
					inputWrapper : settings.inputWrapper,
					markErrorAtHeader : settings.markErrorAtHeader,
					markedHeaderCssClass : settings.markedHeaderCssClass,
			...
			if (settings.validateOnSubmit) {
				...
				$form.submit(function(){
					...
					if(settings.beforeValidate==undefined || settings.beforeValidate($form)) {
						$.fn.yiiactiveform.validate($form, function(data){
							var hasError = false;
							var headerErrors, blockHeader, currentHeader, $header, defaultState;
							$.each(settings.attributes, function(i, attribute){

								if (attribute.markErrorAtHeader) {
									$header = $.fn.yiiactiveform.getRelatedHeader(attribute, $form);
									currentHeader = id + '_' + $header.find(' > span:first-child').text();
									...
									if (defaultState) {
										$header.removeClass(attribute.validatingCssClass+' '+attribute.errorCssClass+' '+attribute.successCssClass);
										defaultState = false;
									}
									if ($.fn.yiiactiveform.updateInput(attribute, data, $form)) ++headerErrors;
									if (headerErrors == 1) {
										$header.removeClass(attribute.validatingCssClass+' '+attribute.errorCssClass+' '+attribute.successCssClass).addClass(attribute.errorCssClass);
										hasError = true;
									}
								} else hasError = $.fn.yiiactiveform.updateInput(attribute, data, $form) || hasError;
							});
			...
			$form.bind('reset',function(){
				...
				setTimeout(function(){
					$.each(settings.attributes, function(i, attribute){
						...
						if (attribute.markErrorAtHeader) {
							var $header = $.fn.yiiactiveform.getRelatedHeader(attribute, $form);
							$header.removeClass(attribute.validatingCssClass+' '+attribute.errorCssClass+' '+attribute.successCssClass);
						}
	...
	$.fn.yiiactiveform.getInputContainer = function(attribute, form) {
		if(attribute.inputContainer == undefined)
			return $('#'+attribute.inputID, form).closest(attribute.inputWrapper);
		else
			//return $(attribute.inputContainer).filter(':has("#'+attribute.inputID+'")').find('#'+attribute.inputID).closest(attribute.inputWrapper);
			return $(attribute.inputContainer).closest(attribute.inputWrapper);
	};

	$.fn.yiiactiveform.getRelatedHeader = function(attribute, form) {
		if(attribute.inputContainer == undefined)
			return $('#'+attribute.inputID, form).closest('div.form-section').prev("div." + attribute.markedHeaderCssClass)
		else return $(attribute.inputContainer).closest('div.form-section').prev("div." + attribute.markedHeaderCssClass)
	};
		...
		$.each(settings.attributes, function(i, attribute){
			if(messages && $.isArray(messages[attribute.id])) {
				$.each(messages[attribute.id],function(j,message){
					content = content + '<li><i class="fa fa-warning" style="padding-right:5px;"></i>' + message + '</li>';
				});
			}
		});
	...
	$.fn.yiiactiveform.defaults = {
		...
		inputWrapper : 'span',
		markErrorAtHeader : false,
		markedHeaderCssClass : 'separator',
